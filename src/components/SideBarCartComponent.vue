                ...
                <div class="order-md-last" v-show="cartLength > 0">
                    ...
                    <ul class="list-group mb-3" v-for="product in cartProducts" :key="product.productId">
                        <li class="list-group-item d-flex justify-content-between lh-sm">
                            <div>
                                <h6 class="my-0 custom-title mb-3" style="font-weight: bold">
                                    {{ product.productName }}
                                </h6>
                                <small class="text-body-secondary custom-quantity">數量: {{ product.quantity }}</small>
                            </div>
                            <div class="text-body-secondary custom-price">
                                $ {{ product.quantity * product.price }}
                                <button
                                    @click="removeFromCart(product)"
                                    class="btn btn-md rounded-circle bg-light border mt-2 ms-3 mb-2"
                                >
...
.cart-badge {
    ...
    top: -5px;
    right: -10px;
    background-color: #fb6571; /* 背景顏色 */
    color: white; /* 文字顏色 */
    border-radius: 50%; /* 圓形 */
    padding: 0.2em 0.5em; /* 內邊距 */
